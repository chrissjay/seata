 ...
 */
package io.seata.rm.datasource.undo.sybase.keyword;
...
/**
 * The type sybase sql keyword checker.
 *
 * @author ccg
 * @date 2019/3/25 sybase keyword checker
 */
public class SybaseKeywordChecker implements KeywordChecker {
    private static volatile KeywordChecker keywordChecker = null;
    private static volatile Set<String> keywordSet = null;

    private SybaseKeywordChecker() {
    }

    /**
     * get instance of type sybase keyword checker
     *
     * @return instance
     */
    public static KeywordChecker getInstance() {
        if (keywordChecker == null) {
            synchronized (SybaseKeywordChecker.class) {
                if (keywordChecker == null) {
                    keywordChecker = new SybaseKeywordChecker();
                    keywordSet = Arrays.stream(SybaseKeyword.values()).map(SybaseKeyword::name).collect(Collectors.toSet());
                }
            }
        }
